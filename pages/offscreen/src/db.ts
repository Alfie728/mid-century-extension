import type { ActionPayload, ScreenshotArtifact, SessionState, UploadJob, VideoChunk } from '@extension/shared';
...
export const saveScreenshot = async (artifact: ScreenshotArtifact) =>
  runTx('screenshots', 'readwrite', store => {
    const record: StoredScreenshot = { ...artifact, createdAt: Date.now() };
    return new Promise<void>((resolve, reject) => {
      const request = store.put(record);
      request.onsuccess = () => resolve();
      request.onerror = () => reject(request.error);
...
export const saveVideoChunk = async (chunk: VideoChunk) =>
  runTx('videoChunks', 'readwrite', store => {
    const record: StoredVideoChunk = { ...chunk, createdAt: Date.now() };
    return new Promise<void>((resolve, reject) => {
      const request = store.put(record);
      request.onsuccess = () => resolve();
      request.onerror = () => reject(request.error);
...
export const saveUploadJob = async (job: UploadJob) =>
  runTx('uploadJobs', 'readwrite', store => {
    const record: StoredUploadJob = { ...job, createdAt: job.createdAt ?? Date.now() };
    return new Promise<void>((resolve, reject) => {
      const request = store.put(record);
      request.onsuccess = () => resolve();
      request.onerror = () => reject(request.error);
...
export const listRecentActions = async (limit = 50): Promise<StoredAction[]> => {
  const db = await getDb();
  return new Promise<StoredAction[]>((resolve, reject) => {
    ...
    cursorRequest.onsuccess = () => {
      ...
      results.push(cursor.value as StoredAction);
      cursor.continue();
    };
  });
};
...
const trimOldestByCreatedAt = async (storeName: StoreName, max: number) => {
  const db = await getDb();
  ...
  try {
    await Promise.all(
      Object.entries(limits).map(async ([storeName, max]) => {
        if (!max) return;
        await trimOldestByCreatedAt(storeName as StoreName, max);
      }),
    );
  } catch (error) {
    console.warn('[CUA][DB] enforceLimits failed', error);
  }
};
